            ...
            if (File.Exists(csvFilePath))
            {
                using (StreamReader reader = new StreamReader(csvFilePath))
                {
                    ...
                    while (reader.Peek() >= 0)
                    {
                        ...
                        /*
                         * Recreating date as dd/MM/20yy HH:mm => short datetime pattern fr-FR
                         */
                        string dateTime = $"{dd.ToString("00")}/{MM.ToString("00")}/20{yy.ToString("00")} {hh.ToString("00")}:{mm.ToString("00")}";

                        /*
                         * Recreating date as MM/dd/20yy HH:mm => short datetime pattern en-US
                         */
                        //string dateTime = $"{MM.ToString("00")}/{dd.ToString("00")}/20{yy.ToString("00")} {hh.ToString("00")}:{mm.ToString("00")}";
        ...
        public static List<Dictionary<string, int[]>> EncodePowerConsumptionData(List<Dictionary<string, string>> data, bool trace = false)
        {
            ...
            foreach (var sequence in data)
            {
                ...
                foreach (var keyValuePair in sequence)
                {
                    ...
                    //DateTime dateTime = DateTime.ParseExact(value, "dd/MM/yyyy HH:mm", CultureInfo.CreateSpecificCulture("fr-FR"));
                    DateTime dateTime = DateTime.ParseExact(value, "dd/MM/yyyy HH:mm", CultureInfo.CreateSpecificCulture("en-US"));
